            ...
            <div class="col-12 col-sm-12 col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <h5>{{$main_name['title']}} <span>(Bölge Düzenle)</span></h5>
                    </div>
                    <div class="card-body">
                        <form action="{{route('crm/destination/update')}}" method="POST" enctype="multipart/form-data">
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="row form-group">
                                        <div class="col-md-12"><label class="control-label">Bölge Resmi Seç</label></div>
                                ...
                                <div class="col-md-6">
                                    <div class="row form-group">
                                        ...
                                        <div class="col-md-12"><img class="img-thumbnail" src="{{!empty($destination['image']) ? Storage::url($destination['image']):Storage::url('noimage-small.jpg')}}" width="120"/></div>
                                        <input name="image" type="hidden" value="{{!empty($destination['image']) ? $destination['image']:'noimage-small.jpg'}}" />
                                    </div>
                                </div>
                            </div>
                                ...
                                @foreach($languages as $key => $lang)
                                    <div class="tab-pane fade {{$i==0? 'active show':null}}" id="lang-{{$lang['id']}}" role="tabpanel" aria-labelledby="lang-{{$lang['code']}}">
                                        <div class="form-group">
                                            <label class="control-label">Bölge Adı ({{$lang['code']}})</label>
                                            <input name="title[{{$lang['id']}}]" class="form-control" type="text" value="{{@$destination['lang'][$key]['name']}}" placeholder="Bölge Adı" required="required">
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Açıklama ({{$lang['code']}})</label>
                                            <textarea name="description[{{$lang['id']}}]" class="form-control editor" id="desc_{{$lang['code']}}">{{@$destination['lang'][$key]['description']}}</textarea>
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Meta Açıklama ({{$lang['code']}})</label>
                                            <input name="meta[{{$lang['id']}}]" class="form-control" type="text" value="{{@$destination['lang'][$key]['meta']}}" placeholder="Meta Açıklama" required="required">
                                        </div>
                                        <div class="form-group">
                                            <label class="control-label">Etiketler ({{$lang['code']}})</label>
                                            <input name="tags[{{$lang['id']}}]" class="form-control" type="text" value="{{@$destination['lang'][$key]['tags']}}" placeholder="Etiketler" required="required">
                                        </div>
                                    </div>
                                    <?php $i++?>
                                @endforeach
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="status" id="status">
                                                <option value="1"{{@$destination['status']==1 ? ' selected':null}}>Aktif</option>
                                                <option value="0"{{@$destination['status']==0 ? ' selected':null}}>Pasif</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
