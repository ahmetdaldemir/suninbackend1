...
@section('title', 'Acentalar - Sunin Turkey')
...
@section('breadcrumb-title')
    <h2>Acentalar<span> </span></h2>
@endsection

@section('breadcrumb-items')
    <li class="breadcrumb-item">Yönetim</li>
    <li class="breadcrumb-item active">Acentalar</li>
@endsection
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="text" name="email" value="{{@$agent['email']}}">
                                    </div>
                                </div>
                                @csrf
                            </div>
                            ...
                            @if($agents)
                                @foreach($agents as $result)
                                    <tr>
                                        ...
                                        <td>{{@$result['email']}}</td>
                                        <td>
                                            <a href="{{'/agents/'.$result['id']}}" class="btn btn-shadow-primary">Düzenle</a>
                                            <a href="{{'/agents/delete/'.$result['id']}}" class="btn btn-shadow-warning delete" data-id="{{$result['id']}}">Sil</a>
                                        </td>
                                    </tr>
                                @endforeach
                            @else
