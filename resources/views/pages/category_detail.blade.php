                    ...
                    <article class="post type-post format-standard hentry article-sq">
                        <div class="entry-content article-content-sq">
                            <div class="article-featured-image-sq">
                                <img  src="{{Storage::url($category['rentcategory']->image)}}" alt="{{$category['lang']->where('lang_id',$lang_id)->first()->title}}">
                            </div>
                            <h2>{{$category['lang']->where('lang_id',$lang_id)->first()->name}}</h2>
                            {!! $category['lang']->where('lang_id',$lang_id)->first()->description !!}
                        </div>
                    </article>
                </div>
            </div>
